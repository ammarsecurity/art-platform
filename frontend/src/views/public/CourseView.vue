<template>
  ...
  <div v-else-if="course" class="pt-24 pb-20">
    <!-- Hero -->
    <div class="relative bg-dark-100 border-b border-dark-300">
      <div class="max-w-7xl mx-auto px-4 py-16 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <h1 class="text-4xl font-bold text-white mb-4 leading-tight">{{ course.title }}</h1>
          <p class="text-gray-400 text-lg leading-relaxed mb-8">{{ course.shortDescription }}</p>

          <div class="flex flex-wrap gap-6 text-sm text-gray-400 mb-8">
            <span>📚 {{ course.lessonCount }} درس</span>
            <span>⏱ {{ Math.floor(course.durationMinutes / 60) }} ساعة</span>
            <span>👥 {{ course.enrollmentCount }} طالب</span>
            <span>💰 {{ course.price === 0 ? 'مجاني' : `${course.price} ر.س` }}</span>
          </div>
    ...
    <div class="max-w-7xl mx-auto px-4 py-16 grid lg:grid-cols-3 gap-10">
      ...
      <div class="lg:col-span-2">
        <h2 class="text-2xl font-bold text-white mb-8">محتوى الدورة</h2>
        <div class="space-y-3">
          <div v-for="(lesson, i) in course.lessons" :key="lesson.id"
            ...
            <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0"
              :class="lesson.isCompleted ? 'bg-green-500/20 text-green-400' : 'bg-dark-200 text-gray-400'">
              {{ lesson.isCompleted ? '✓' : i + 1 }}
            </div>

            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <h4 class="text-white font-medium text-sm line-clamp-1">{{ lesson.title }}</h4>
                <span v-if="lesson.isPreview" class="badge bg-gold/20 text-gold border-gold/30 text-xs">معاينة</span>
              </div>
              <span class="text-gray-500 text-xs">⏱ {{ lesson.durationMinutes }} دقيقة</span>
            </div>
      ...
      <div class="space-y-6">
        <div class="card p-6">
          <h3 class="text-white font-bold mb-4">تفاصيل الدورة</h3>
          <div class="space-y-4 text-sm">
            <div class="flex justify-between">
              <span class="text-gray-400">المستوى</span>
              <span class="text-white">{{ levelLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-400">عدد الدروس</span>
              <span class="text-white">{{ course.lessonCount }} درس</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-400">المدة الكلية</span>
              <span class="text-white">{{ Math.floor(course.durationMinutes / 60) }}س {{ course.durationMinutes % 60 }}د</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-400">الطلاب المسجلين</span>
              <span class="text-white">{{ course.enrollmentCount }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-400">السعر</span>
              <span class="text-gold font-bold">{{ course.price === 0 ? 'مجاني' : `${course.price} ر.س` }}</span>
            </div>
          </div>
        </div>
      </div>
