<template>
  <div class="min-h-screen py-24 px-4">
    <div class="max-w-4xl mx-auto">
      ...
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-fg mb-2">دوراتي</h1>
        <p class="text-fg-mute">الدورات التي سجّلت فيها</p>
      </div>

      <!-- Loading -->
      <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div v-for="i in 4" :key="i" class="card p-5 animate-pulse">
          <div class="h-40 bg-line rounded-xl mb-4"></div>
          <div class="h-4 bg-line rounded w-3/4 mb-2"></div>
          <div class="h-3 bg-line rounded w-1/2"></div>
        </div>
      </div>
      ...
      <div v-else-if="!courses.length" class="text-center py-20">
        ...
        <h2 class="text-xl font-semibold text-fg mb-2">لم تسجّل في أي دورة بعد</h2>
        <p class="text-fg-mute mb-6">اكتشف دوراتنا وابدأ رحلتك الفنية</p>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div v-for="course in courses" :key="course.id" class="card overflow-hidden group">
          <!-- Thumbnail -->
          <div class="relative h-44 bg-line overflow-hidden">
            <img v-if="course.thumbnailUrl" :src="resolveMediaUrl(course.thumbnailUrl)" :alt="course.title"
          ...
          <div class="p-5">
            <h3 class="text-fg font-semibold text-lg mb-1 line-clamp-1">{{ course.title }}</h3>
            <p class="text-fg-mute text-sm mb-4 line-clamp-2">{{ course.shortDescription || course.description }}</p>

            <!-- Progress -->
            <div class="mb-4">
              <div class="flex justify-between text-xs text-fg-mute mb-1.5">
                <span>التقدم</span>
            ...
            <!-- Stats row -->
            <div class="flex items-center gap-4 text-xs text-fg-dim mb-4">
