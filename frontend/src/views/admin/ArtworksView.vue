<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-3xl font-bold text-fg">الأعمال الفنية</h1>
        <p class="text-fg-mute mt-1">إدارة معرض الأعمال الفنية</p>
      </div>
    ...
    <div class="card overflow-hidden">
      <div v-if="loading" class="p-8 text-center text-fg-dim">
      ...
      <table v-else class="w-full">
        <thead>
          <tr class="border-b border-line text-right">
            <th v-for="h in headers" :key="h" class="px-4 py-3 text-xs text-fg-mute font-medium">{{ h }}</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-line">
          <tr v-for="artwork in artworks" :key="artwork.id"
            ...
            <td class="px-4 py-3 text-fg-mute text-sm">{{ artwork.categoryName }}</td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-fg-mute text-xs">{{ formatDate(artwork.createdAt) }}</td>
            <td class="px-4 py-3">
              <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <RouterLink :to="`/admin/artworks/${artwork.id}/edit`"
                  class="px-3 py-1 text-xs bg-line hover:bg-input text-fg rounded-lg transition-colors">تعديل</RouterLink>
      ...
      <div v-if="!loading && !artworks.length" class="text-center py-16 text-fg-dim">
...
const statusClass = s => ({
  ...
  Archived: 'bg-gray-500/20 text-fg-mute border border-gray-500/30',
}[s] || '')
