<template>
  <div class="space-y-8">
    <h1 class="text-2xl font-bold text-white">إدارة الصفحات القانونية</h1>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- Terms of Use -->
      <div class="card p-6 space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold text-white">شروط الاستخدام</h2>
        ...
        <button @click="save('terms_of_use', terms)" :disabled="saving.terms" class="btn-primary w-full justify-center">
          <span v-if="saving.terms" class="inline-block w-4 h-4 border-2 border-dark/30 border-t-dark rounded-full animate-spin ml-2"></span>
      ...
      <div class="card p-6 space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold text-white">سياسة الخصوصية</h2>
        ...
        <button @click="save('privacy_policy', privacy)" :disabled="saving.privacy" class="btn-primary w-full justify-center">
          <span v-if="saving.privacy" class="inline-block w-4 h-4 border-2 border-dark/30 border-t-dark rounded-full animate-spin ml-2"></span>
    ...
    <!-- Markdown Guide -->
    <div class="card p-5 bg-dark-100/50">
      <h3 class="text-sm font-semibold text-gold mb-3">دليل Markdown</h3>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-gray-400 font-mono">
        <span><span class="text-white"># عنوان</span> → عنوان رئيسي</span>
        <span><span class="text-white">## عنوان</span> → عنوان ثانوي</span>
        <span><span class="text-white">**نص**</span> → نص عريض</span>
        <span><span class="text-white">سطر فارغ</span> → فقرة جديدة</span>
      </div>
    </div>
  </div>
</template>
