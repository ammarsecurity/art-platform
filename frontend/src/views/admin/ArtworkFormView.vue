<template>
  <div>
    <div class="flex items-center gap-4 mb-8">
      <RouterLink to="/admin/artworks" class="text-gray-400 hover:text-white transition-colors">← رجوع</RouterLink>
      <h1 class="text-3xl font-bold text-white">{{ isEdit ? 'تعديل العمل الفني' : 'إضافة عمل فني جديد' }}</h1>
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="grid lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        <div class="card p-6 space-y-5">
          <div>
            <label class="block text-sm text-gray-400 mb-2">عنوان العمل *</label>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm text-gray-400 mb-2">الخامة / الأسلوب</label>
              <input v-model="form.medium" type="text" class="input-field" placeholder="زيت على قماش">
            </div>
            <div>
              <label class="block text-sm text-gray-400 mb-2">الأبعاد</label>
              <input v-model="form.dimensions" type="text" class="input-field" placeholder="60 × 80 سم">
            </div>
          </div>
          <div>
            <label class="block text-sm text-gray-400 mb-2">التاجات (افصل بمسافة)</label>
      ...
      <div class="space-y-6">
        <!-- Image upload -->
        <div class="card p-6">
          <label class="block text-sm text-gray-400 mb-3">صورة العمل {{ isEdit ? '(اتركها فارغة للإبقاء على الصورة الحالية)' : '*' }}</label>
          <div
            class="border-2 border-dashed border-dark-300 rounded-xl p-8 text-center cursor-pointer hover:border-gold/40 transition-colors"
          ...
          >
            ...
            <div v-else>
              ...
              <p class="text-gray-400 text-sm">اضغط لرفع الصورة</p>
              <p class="text-gray-600 text-xs mt-1">JPG, PNG, WebP — بحد أقصى 10 ميجابايت</p>
            </div>
          </div>
        ...
        <div class="card p-6 space-y-5">
          ...
          <div>
            <label class="block text-sm text-gray-400 mb-2">السنة</label>
          ...
          <label class="flex items-center gap-3 cursor-pointer">
            <div class="relative">
              ...
              <div class="w-10 h-6 bg-dark-300 rounded-full peer-checked:bg-gold transition-colors"></div>
              <div class="absolute top-1 right-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-[-16px]"></div>
            </div>
            <span class="text-sm text-gray-400">تمييز هذا العمل ⭐</span>
          </label>
        </div>
